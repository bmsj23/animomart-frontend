...
import { LayoutDashboard, Package, ShoppingCart, LogOut, ChevronLeft, Menu, X } from 'lucide-react';
...
const SellerLayout = ({ children }) => {
  ...
  const navItems = [
    { path: '/seller/dashboard', label: 'Dashboard', icon: LayoutDashboard, color: 'green' },
    { path: '/seller/products', label: 'Products', icon: Package, color: 'blue' },
    { path: '/seller/orders', label: 'Orders', icon: ShoppingCart, color: 'purple' },
  ];
            ...
            <>
              <Link to="/" className="flex items-center gap-3 group hover:cursor-pointer">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 font-medium">Seller Panel</p>
                </div>
              </Link>
              <button
          ...
          {isSidebarOpen ? (
            <div className="space-y-3">
              <div className="flex items-center gap-3 px-3 py-2">
                <img
                  src={user?.profilePicture || user?.picture || 'https://via.placeholder.com/40'}
                  alt="Profile"
            ...
            <button
              onClick={handleLogout}
              className="w-full p-3 text-red-600 hover:bg-red-50 rounded-xl transition-colors flex items-center justify-center hover:cursor-pointer"
            >
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 h-16 bg-white border-b border-gray-200 flex items-center justify-between px-4 z-50">
        <Link to="/" className="flex items-center gap-2 hover:cursor-pointer">
          <div className="w-8 h-8 bg-white rounded-lg flex items-center justify-center">
            <img src="/assets/tr_animomart_1.png" alt="AnimoMart Logo" className="w-6 h-6 object-contain" />
          </div>
              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    ...
                    to={item.path}
                    onClick={() => setIsMobileSidebarOpen(false)}
                    className={`flex items-center gap-4 px-4 py-3.5 rounded-xl transition-all hover:cursor-pointer ${
                      active
