...
const ReviewsTab = ({
  ...
  if (authoredReviews.length === 0) {
    return (
      <div className="py-8 text-center">
        <p className="text-gray-700 mb-4">You haven't written any reviews yet.</p>
        <Link to="/profile?tab=purchases" className="px-4 py-2 bg-green-600 text-white rounded-md">View purchases</Link>
      </div>
    );
  }
        ...
        <div key={r._id || r.id || `${r.product?._id || r.product?.id}-${r.createdAt}` } className="bg-white border border-gray-100 rounded-lg p-4">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-700 mb-2">{r.reviewText || r.body || r.comment || r.text}</p>
              ...
              <div className="flex items-center justify-between mt-3">
                <p className="text-xs text-gray-400">
                  {r.createdAt && `Reviewed on ${new Date(r.createdAt).toLocaleDateString()}`}
                </p>
