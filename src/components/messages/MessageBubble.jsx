import { MoreVertical, Check, CheckCheck, Trash2 } from 'lucide-react';
      ...
      <div className={`max-w-[85%] sm:max-w-xs lg:max-w-md ${isOwnMessage ? 'order-2' : 'order-1'} relative`}>
        ...
        {isOwnMessage && (
          <div ref={menuRef} className="absolute -left-8 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            {/* Dropdown menu */}
            {showMenu && (
              <div className="absolute right-full top-1/2 -translate-y-1/2 mr-2 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-10 min-w-[120px] hover:bg-red-50">
                <button
                  onClick={() => {
                    setShowDeleteModal(true);
                    setShowMenu(false);
                  }}
                  className="w-full px-4 py-2 text-left text-sm text-red-600 transition-colors hover:cursor-pointer"
                >
                  Delete
                </button>
              </div>
        ...
        actions={
          <>
            <button
              ...
              className="px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100 rounded-lg transition-colors hover:cursor-pointer"
            >
              Cancel
            </button>
            <button
              ...
              className="px-4 py-2 text-sm font-medium text-white bg-red-600 hover:bg-red-700 rounded-lg transition-colors hover:cursor-pointer"
            >
              Delete
            </button>
          </>
